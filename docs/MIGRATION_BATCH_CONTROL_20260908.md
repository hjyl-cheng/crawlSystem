...
## Full Crawl capacity: 40 workers

At the next user request, migration workers increased from 30 to 40 (`worker-channel-31` through `-40`), incremental remained 20, and Rota channel slots increased from 51 to 61. The Compose scale overrides now reflect 40/20 and 61. A temporary maintenance drain was used for the Rota restart: the controller was gracefully stopped and queues paused until both BullMQ active work and Rota active tasks reached zero. The user requested ending the current batch during maintenance; the controller and queues were restored for recovery/finalization without starting another batch.

Final batch `migration-ae60f249-79c8-4ddb-8911-42022b8f401e`: ended, 503 success, 5 dormant, 396,312 released. All 61 channel-slot leases valid; 40 migration and 20 incremental containers running. Worker-channel-31 initially encountered ECONNREFUSED while Rota was starting, automatically restarted once, and then logged rota_slot_ready / worker started. No second expansion is needed. The ended batch's crawl queue is now managed by the normal idle controller policy; a new batch is needed for further migration. Runtime log: `/tmp/scale-migration-40.log`.

## Agent capacity: 3 workers

At user request, Agent expanded to `worker-agent-1`, `-2`, `-3`, using code image `pachongsys-aa939bf`. The original worker drained and was upgraded; two new workers use independent hostnames with the existing local-offline configuration. Enabled Agent config 1 now has max_workers=3, while batch_size remains 20. The existing controller synchronized youtube-agent-batch global concurrency to 3. All three workers registered and ran with zero restarts. The crawler Compose scale override now includes worker-agent replicas=3; retain max_workers=3 in the persisted crawler.agent_configs row when restoring database configuration.

At final audit there were no active Agent jobs or started items in the latest migration batch, so this rollout verifies configuration/registration, not a measured threefold throughput increase. Further throughput measurement requires a subsequent active batch. Deployment log: `/tmp/scale-agent-3.log`.
