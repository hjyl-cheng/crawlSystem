...
CREATE TABLE public.content_snapshots (
    ...
    CONSTRAINT content_snapshots_access_shape CHECK (((access_status = ANY (ARRAY['public'::text, 'login_required'::text, 'members_only'::text, 'unavailable'::text, 'unknown'::text])) AND ((access_status_source IS NULL) OR (btrim(access_status_source) <> ''::text)) AND ((source_position IS NULL) OR (source_position > 0)) AND ((comments_disabled IS DISTINCT FROM true) OR ((comment_count IS NULL) AND (comment_count_status = 'unavailable'::text))))),
