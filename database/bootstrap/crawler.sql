...
CREATE TABLE crawler.channel_runs (
    ...
    detail_job_epoch bigint DEFAULT 0 NOT NULL,
    detail_active_job_epoch bigint,
    CONSTRAINT channel_runs_crawl_mode_check CHECK ((crawl_mode = ANY (ARRAY['full'::text, 'incremental'::text]))),
    CONSTRAINT channel_runs_detail_active_job_check CHECK (((detail_job_epoch >= 0) AND (((detail_active_job_id IS NULL) AND (detail_active_job_attempt IS NULL) AND (detail_active_scope_key IS NULL) AND (detail_active_job_epoch IS NULL)) OR ((detail_active_job_id IS NOT NULL) AND (detail_active_job_attempt IS NOT NULL) AND (detail_active_job_attempt > 0) AND (detail_active_scope_key IS NOT NULL) AND (detail_active_job_epoch IS NOT NULL) AND (detail_active_job_epoch = detail_job_epoch))))),
