...
FEATURE_IMAGE_TAG="$(awk -F= '$1 == "QY_FEATURE_ENGINE_IMAGE_TAG" { print $2; exit }' "${ENV_FILE}")"
if [[ -z "${FEATURE_IMAGE_TAG}" || "${FEATURE_IMAGE_TAG}" == "latest" || "${FEATURE_IMAGE_TAG}" == "local" ]]; then
  echo "QY_FEATURE_ENGINE_IMAGE_TAG must pin an immutable pachongsys image" >&2
  exit 1
fi
...
exec docker compose \
  ...
  -f "${ROOT_DIR}/deploy/compose.qy-feature-bridge-runtime.yml" \
  "$@"
