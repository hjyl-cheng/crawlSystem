...
const PixelBlast: React.FC<PixelBlastProps> = ({
  ...
  useEffect(() => {
    ...
    type EffectUniforms = { uniforms: Map<string, { value: number }> };
    ...
    if (mustReinit) {
      ...
      const animate = () => {
        ...
        if (liquidEffect) {
          const uTime = (liquidEffect as unknown as EffectUniforms).uniforms.get('uTime');
          if (uTime) uTime.value = uniforms.uTime.value;
        }
        if (composer) {
          if (touch) touch.update();
          composer.passes.forEach(p => {
            const effs = (p as unknown as { effects?: EffectUniforms[] }).effects;
            if (effs)
              effs.forEach(eff => {
      ...
      if (t.liquidEffect) {
        const fx = t.liquidEffect as unknown as EffectUniforms;
        const uStrength = fx.uniforms.get('uStrength');
        if (uStrength) uStrength.value = liquidStrength;
        const uFreq = fx.uniforms.get('uFreq');
