...
type Proxy struct {
	...
	FailureEpisodeKind  *string    `json:"failure_episode_kind,omitempty"`
	...
	BaseHealthStatus    *string    `json:"base_health_status,omitempty"`
	YouTubeHealthStatus *string    `json:"youtube_health_status,omitempty"`
...
type CreateProxyRequest struct {
	...
	NodeIdentity string   `json:"-"`                   // normalized internal uniqueness key
}
...
// BulkTagProxyRequest represents a request to add or remove tags on multiple proxies.
...
type ArchiveProxyRequest struct {
	Reason string `json:"reason,omitempty"`
}

type BulkArchiveProxyRequest struct {
	IDs    []int  `json:"ids" validate:"required,min=1"`
	Reason string `json:"reason,omitempty"`
}

type BulkRestoreProxyRequest struct {
	IDs []int `json:"ids" validate:"required,min=1"`
}
