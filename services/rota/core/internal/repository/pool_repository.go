...
// AddProxies adds proxy IDs to a pool (idempotent). Missing proxy IDs are
// skipped by the INSERT ... SELECT instead of failing the whole request.
func (r *PoolRepository) AddProxies(ctx context.Context, poolID int, proxyIDs []int) error {
	...
	_, err := r.db.Pool.Exec(ctx, `
		...
		ON CONFLICT DO NOTHING`, poolID, proxyIDs)
	if err != nil {
...
// rebuildMembership atomically replaces an automatically managed pool's
// membership. The final status predicate is deliberate: a proxy archived
// after filter evaluation must not be reintroduced by the rebuild.
func (r *PoolRepository) rebuildMembership(ctx context.Context, poolID int, ids []int) ([]int, error) {
	tx, err := r.db.Pool.Begin(ctx)
	if err != nil {
		return nil, fmt.Errorf("failed to begin pool membership transaction: %w", err)
	}
	defer tx.Rollback(ctx) //nolint:errcheck // no-op after a successful commit

	if _, err := tx.Exec(ctx, `DELETE FROM pool_proxies WHERE pool_id = $1`, poolID); err != nil {
		return nil, fmt.Errorf("failed to clear pool membership: %w", err)
	}

	inserted := make([]int, 0, len(ids))
	if len(ids) > 0 {
		rows, err := tx.Query(ctx, `
			...
			RETURNING proxy_id`, poolID, ids)
		if err != nil {
			return nil, fmt.Errorf("failed to insert pool membership: %w", err)
		}
		for rows.Next() {
			var id int
			if err := rows.Scan(&id); err != nil {
				rows.Close()
				return nil, fmt.Errorf("failed to scan inserted pool membership: %w", err)
			}
	...
	if err := tx.Commit(ctx); err != nil {
		return nil, fmt.Errorf("failed to commit pool membership: %w", err)
	}
	return inserted, nil
}
...
func (r *PoolRepository) SetGeoFilters(ctx context.Context, poolID int, filters []models.GeoFilter) error {
	tx, err := r.db.Pool.Begin(ctx)
	if err != nil {
		return fmt.Errorf("failed to begin geo filters transaction: %w", err)
	}
	defer tx.Rollback(ctx) //nolint:errcheck // no-op after a successful commit

	if _, err := tx.Exec(ctx, `DELETE FROM pool_geo_filters WHERE pool_id=$1`, poolID); err != nil {
		return fmt.Errorf("failed to clear geo filters: %w", err)
	}
	for _, f := range filters {
		_, err := tx.Exec(ctx,
			`INSERT INTO pool_geo_filters (pool_id, country_code, city_name) VALUES ($1,$2,$3) ON CONFLICT DO NOTHING`,
			poolID, f.CountryCode, f.CityName)
		if err != nil {
			return fmt.Errorf("failed to insert geo filter: %w", err)
		}
	}
	if err := tx.Commit(ctx); err != nil {
		return fmt.Errorf("failed to commit geo filters: %w", err)
	}
	return nil
}
...
func (r *PoolRepository) SyncPoolByGeo(ctx context.Context, pool models.ProxyPool) (int, error) {
	...
	for _, f := range filters {
		...
		if f.CityName != "" {
			rows, qerr = r.db.Pool.Query(ctx,
				`SELECT id FROM proxies WHERE status <> 'archived' AND country_code=$1 AND city_name ILIKE $2`,
				f.CountryCode, "%"+f.CityName+"%")
		} else {
			rows, qerr = r.db.Pool.Query(ctx,
				`SELECT id FROM proxies WHERE status <> 'archived' AND country_code=$1`,
				f.CountryCode)
		}
		if qerr != nil {
	...
	inserted, err := r.rebuildMembership(ctx, pool.ID, ids)
	if err != nil {
		return 0, err
	}
...
func (r *PoolRepository) SetISPFilters(ctx context.Context, poolID int, isps []string) error {
	tx, err := r.db.Pool.Begin(ctx)
	if err != nil {
		return fmt.Errorf("failed to begin ISP filters transaction: %w", err)
	}
	defer tx.Rollback(ctx) //nolint:errcheck // no-op after a successful commit

	if _, err := tx.Exec(ctx, `DELETE FROM pool_isp_filters WHERE pool_id=$1`, poolID); err != nil {
		return fmt.Errorf("failed to clear ISP filters: %w", err)
	}
	for _, isp := range isps {
		if _, err := tx.Exec(ctx,
			`INSERT INTO pool_isp_filters (pool_id, isp) VALUES ($1,$2) ON CONFLICT DO NOTHING`,
			poolID, isp); err != nil {
			return fmt.Errorf("failed to insert ISP filter: %w", err)
		}
	}
	if err := tx.Commit(ctx); err != nil {
		return fmt.Errorf("failed to commit ISP filters: %w", err)
	}
	return nil
}
...
func (r *PoolRepository) SetTagFilters(ctx context.Context, poolID int, tags []string) error {
	tx, err := r.db.Pool.Begin(ctx)
	if err != nil {
		return fmt.Errorf("failed to begin tag filters transaction: %w", err)
	}
	defer tx.Rollback(ctx) //nolint:errcheck // no-op after a successful commit

	if _, err := tx.Exec(ctx, `DELETE FROM pool_tag_filters WHERE pool_id=$1`, poolID); err != nil {
		return fmt.Errorf("failed to clear tag filters: %w", err)
	}
	for _, tag := range tags {
		if _, err := tx.Exec(ctx,
			`INSERT INTO pool_tag_filters (pool_id, tag) VALUES ($1,$2) ON CONFLICT DO NOTHING`,
			poolID, tag); err != nil {
			return fmt.Errorf("failed to insert tag filter: %w", err)
		}
	}
	if err := tx.Commit(ctx); err != nil {
		return fmt.Errorf("failed to commit tag filters: %w", err)
	}
	return nil
}
...
func (r *PoolRepository) SyncPoolByFilters(ctx context.Context, pool models.ProxyPool) (int, []int, error) {
	...
	for _, f := range geoFilters {
		if f.CityName != "" {
			if err := addIDs(`SELECT id FROM proxies WHERE status <> 'archived' AND country_code=$1 AND city_name ILIKE $2`,
				f.CountryCode, "%"+f.CityName+"%"); err != nil {
				return 0, nil, err
			}
		} else {
			if err := addIDs(`SELECT id FROM proxies WHERE status <> 'archived' AND country_code=$1`, f.CountryCode); err != nil {
				return 0, nil, err
			}
		}
	}
	...
	for _, isp := range ispFilters {
		if err := addIDs(`SELECT id FROM proxies WHERE status <> 'archived' AND isp ILIKE $1`, "%"+isp+"%"); err != nil {
			return 0, nil, err
		}
	}
	...
	if len(tagFilters) > 0 {
		rows, err := r.db.Pool.Query(ctx,
			`SELECT id FROM proxies WHERE status <> 'archived' AND tags @> $1::text[]`, tagFilters)
		if err != nil {
	...
	newIDs := make([]int, 0, len(inserted))
	for _, id := range inserted {
		if !currentSet[id] {
			newIDs = append(newIDs, id)
		}
	}
	return len(inserted), newIDs, nil
}
...
// UpdateAlertRule updates an alert rule owned by the specified pool.
...
// DeleteAlertRule deletes an alert rule owned by the specified pool.
...
// GetDistinctISPs returns filter-builder ISP choices from repository-owned SQL.
...
// GetDistinctTags returns filter-builder tag choices from repository-owned SQL.
