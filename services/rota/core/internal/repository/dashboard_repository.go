	...
	query := `
		WITH current_stats AS (
			SELECT
				...
				COUNT(*) FILTER (WHERE status = 'failed') as failed_proxies,
				COUNT(*) FILTER (WHERE status = 'idle') as pending_proxies,
				COUNT(*) FILTER (WHERE status = 'archived') as archived_proxies,
