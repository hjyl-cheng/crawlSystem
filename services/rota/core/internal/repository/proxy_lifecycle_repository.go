...
func (r *ProxyRepository) ApplyHealthVerdict(
	...
	err := pgx.BeginFunc(ctx, r.db.Pool, func(tx pgx.Tx) error {
		...
		if current.Status == proxylifecycle.StatusArchived ||
		...
		_, err = tx.Exec(ctx, `
			UPDATE proxies
			SET status = $2,
			    cooldown_until = CASE WHEN $7 THEN NULL ELSE cooldown_until END,
			    failed_since = $3,
		...
		`, proxyID, string(decision.Status), decision.FailedSince, decision.ContinuousFailedSince,
			...
			evidence.CheckedAt, evidence.Verdict.Healthy,
...
func (r *ProxyRepository) ClaimDueHealthChecks(ctx context.Context, limit int) ([]*models.Proxy, error) {
	...
	rows, err := r.db.Pool.Query(ctx, `
		WITH due AS (
			SELECT id
			FROM proxies
			WHERE (status IN ('idle', 'failed') OR (status = 'active' AND revalidation_required))
			  AND next_health_check_at IS NOT NULL
			  AND next_health_check_at <= NOW()
			ORDER BY next_health_check_at, id
	...
	`, limit)
	if err != nil {
		return nil, fmt.Errorf("claim due health checks: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			return nil, fmt.Errorf("scan due health check: %w", err)
		}
