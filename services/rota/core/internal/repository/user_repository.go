...
// HasAny reports whether Proxy User authentication has been configured. Disabled
// users still count: their presence must not turn the forward proxy into an open proxy.
func (r *UserRepository) HasAny(ctx context.Context) (bool, error) {
	var exists bool
	if err := r.db.Pool.QueryRow(ctx, `SELECT EXISTS (SELECT 1 FROM proxy_users)`).Scan(&exists); err != nil {
		return false, fmt.Errorf("check proxy users: %w", err)
	}
	return exists, nil
}
...
func (r *UserRepository) Update(ctx context.Context, id int, req models.UpdateProxyUserRequest) (*models.ProxyUser, error) {
	...
	if mainPoolID != nil && *mainPoolID <= 0 {
		mainPoolID = nil
	}
	...
	err := r.db.Pool.QueryRow(ctx, `
		UPDATE proxy_users SET
			...
			main_pool_id        = CASE WHEN $3 THEN $4::INTEGER ELSE main_pool_id END,
			fallback_pool_ids   = COALESCE($5::INTEGER[], fallback_pool_ids),
		...
		RETURNING id, username, enabled, main_pool_id, fallback_pool_ids, max_retries,
		          COALESCE(requests_per_minute, 0), created_at, updated_at
	`, hashPtr, req.Enabled, req.HasMainPoolID(), mainPoolID, req.FallbackPoolIDs,
		req.MaxRetries, req.RequestsPerMinute, id,
	).Scan(&u.ID, &u.Username, &u.Enabled, &u.MainPoolID, &u.FallbackPoolIDs,
		&u.MaxRetries, &u.RequestsPerMinute, &u.CreatedAt, &u.UpdatedAt)
