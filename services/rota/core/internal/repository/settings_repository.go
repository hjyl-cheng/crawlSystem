...
// SetGeoIPLastUpdated changes only the service-owned update timestamp.
func (r *SettingsRepository) SetGeoIPLastUpdated(ctx context.Context, updatedAt time.Time) error {
	command, err := r.db.Pool.Exec(ctx, `
		UPDATE settings
		SET value = jsonb_set(value, '{last_updated_at}', to_jsonb($1::text), true),
		    updated_at = NOW()
		WHERE key = 'geoip'
	`, updatedAt.UTC().Format(time.RFC3339Nano))
	if err != nil {
		return fmt.Errorf("failed to update geoip timestamp: %w", err)
	}
	if command.RowsAffected() != 1 {
		return fmt.Errorf("geoip setting not found")
	}
	return nil
}
...
func (r *SettingsRepository) Reset(ctx context.Context) error {
	defaults := map[string]map[string]any{
		...
		"geoip": {
			"provider":              "local",
			"maxmind_license_key":   "",
			"maxmind_db_path":       "/app/geoip/managed/GeoLite2-City.mmdb",
			"maxmind_url":           "",
			"auto_update":           false,
