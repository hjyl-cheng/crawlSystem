...
func NewWebSocketHandler(
	...
	handler := &WebSocketHandler{
		...
		sessions:       make(map[*websocket.Conn]context.CancelFunc),
...
func (h *WebSocketHandler) beginSession(r *http.Request, conn *websocket.Conn) (context.Context, context.CancelFunc, bool) {
	...
	if h.sessions == nil {
		h.sessions = make(map[*websocket.Conn]context.CancelFunc)
	}
	if h.sessionsDone == nil {
		h.sessionsDone = make(chan struct{})
	}
	h.sessions[conn] = cancel
	h.sessionWG.Add(1)
	return ctx, cancel, true
}

func (h *WebSocketHandler) endSession(conn *websocket.Conn) {
	h.sessionMu.Lock()
...
// Shutdown rejects new upgrades, cancels and closes every hijacked connection,
// and waits for their handlers and child goroutines to finish.
func (h *WebSocketHandler) Shutdown(ctx context.Context) error {
	...
	sessions := make(map[*websocket.Conn]context.CancelFunc, len(h.sessions))
	for conn, cancel := range h.sessions {
		sessions[conn] = cancel
	}
	h.sessionMu.Unlock()
...
// checkOrigin rejects cross-site browser WebSocket requests unless the Origin
// is same-origin or explicitly allowed by the CORS allowlist.
...
func (h *WebSocketHandler) DashboardWebSocket(w http.ResponseWriter, r *http.Request) {
	...
	ctx, cancel, accepted := h.beginSession(r, conn)
	if !accepted {
		_ = conn.Close()
		return
	}
...
func (h *WebSocketHandler) LogsWebSocket(w http.ResponseWriter, r *http.Request) {
	...
	workers.Go(func() {
		...
		for {
			...
			if err != nil {
				// A Gorilla WebSocket read is no longer usable after a deadline.
	...
	for {
		select {
		case <-pingTicker.C:
			// Control writes may run concurrently with the single data writer.
