...
func NewAuthMiddleware(settings models.AuthenticationSettings) *AuthMiddleware {
	return &AuthMiddleware{
		...
		usernameDigest: sha256.Sum256([]byte(settings.Username)),
		passwordDigest: sha256.Sum256([]byte(settings.Password)),
	}
}

// IsEnabled reports whether legacy single-user authentication is enforcing.
...
func (m *AuthMiddleware) UpdateSettings(settings models.AuthenticationSettings) {
	...
	m.usernameDigest = sha256.Sum256([]byte(settings.Username))
	m.passwordDigest = sha256.Sum256([]byte(settings.Password))
}
...
func (m *AuthMiddleware) HandleRequest(req *http.Request) (*http.Request, *http.Response) {
	m.mu.RLock()
	enabled := m.enabled
	usernameDigest := m.usernameDigest
	...
	// Hashing first makes both constant-time comparisons fixed-length even when
	// the supplied username or password has a different length.
	suppliedUsername := sha256.Sum256([]byte(username))
	suppliedPassword := sha256.Sum256([]byte(password))
	userMatch := subtle.ConstantTimeCompare(suppliedUsername[:], usernameDigest[:])
	passwordMatch := subtle.ConstantTimeCompare(suppliedPassword[:], passwordDigest[:])
...
func (m *RateLimitMiddleware) allow(clientIP string) bool {
	...
	if !m.enabled || m.interval <= 0 || m.maxRequests <= 0 {
		return true
	}
...
// getClientIP uses only the socket peer. Forwarded headers on a forward proxy
// are caller-controlled and cannot safely identify the rate-limit bucket.
