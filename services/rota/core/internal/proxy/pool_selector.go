...
// Refresh reloads only verified active proxies that belong to this pool.
func (ps *PoolSelector) Refresh(ctx context.Context) error {
	rows, err := ps.db.Pool.Query(ctx, `
		...
		LEFT JOIN proxy_running_slots running_slot
		  ON running_slot.pool_id = pp.pool_id AND running_slot.proxy_id = p.id
		WHERE pp.pool_id = $1
		  AND (
		    (p.status = 'active' AND p.revalidation_required=false
		      AND (p.cooldown_until IS NULL OR p.cooldown_until <= NOW()))
		    OR (
		      running_slot.worker_id IS NOT NULL
		      AND running_slot.lease_until > NOW()
		    )
		  )
		ORDER BY p.id
	`, ps.poolID)
...
// HasActive returns true if the pool currently has at least one active proxy.
...
func (ps *PoolSelector) proxyIDs() []int {
	ps.mu.Lock()
	defer ps.mu.Unlock()
	ids := make([]int, 0, len(ps.proxies))
