...
func (s *RandomSelector) Refresh(ctx context.Context) error {
	...
	if err != nil && !errors.Is(err, ErrNoActiveProxies) {
		return err
	}
...
// LeastConnectionsSelector selects the proxy with the lowest live selection count.
...
// Select returns the proxy with the lowest live in-memory selection count.
func (s *LeastConnectionsSelector) Select(_ context.Context) (*models.Proxy, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	...
	for _, proxy := range s.proxies[1:] {
		if count := s.counts[proxy.ID]; count < minCount {
			minProxy = proxy
			minCount = count
		}
	}
...
func (s *LeastConnectionsSelector) Refresh(ctx context.Context) error {
	...
	s.mu.Lock()
	s.countsMu.Lock()
	liveCounts := make(map[int]int64, len(proxies))
	for _, proxy := range proxies {
		if count, ok := s.counts[proxy.ID]; ok {
			liveCounts[proxy.ID] = count
		}
	}
	s.proxies = proxies
	s.counts = liveCounts
...
func (b *BaseSelector) loadActiveProxiesWithSettings(ctx context.Context, settings *models.RotationSettings) ([]*models.Proxy, error) {
	// Only verified active proxies can be allocated.
	query := `
		SELECT
	...
	if len(proxies) == 0 {
		return proxies, ErrNoActiveProxies
	}

	return proxies, nil
}

var ErrNoActiveProxies = errors.New("no active proxies found matching filters")
