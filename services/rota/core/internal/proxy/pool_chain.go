...
// HasExactProxy proves that this managed chain exposes exactly one route and
// that route is the endpoint reserved by the control plane.
func (c *PoolChain) HasExactProxy(expectedProxyID int) bool {
	if c == nil || expectedProxyID <= 0 {
		return false
	}
	count := 0
	for _, selector := range c.selectors {
		...
		for _, proxyID := range selector.proxyIDs() {
			count++
			if proxyID != expectedProxyID || count > 1 {
				return false
			}
		}
	}
...
func (c *PoolChain) RefreshExact(ctx context.Context, expectedProxyID int) error {
	if c == nil {
		return fmt.Errorf("managed PoolChain is unavailable")
	}
	for _, selector := range c.selectors {
		if selector == nil {
			continue
	...
	if !c.HasExactProxy(expectedProxyID) {
		return fmt.Errorf("managed PoolChain does not contain exactly proxy %d", expectedProxyID)
	}
	return nil
}
...
// markFailed temporarily removes a proxy only after repeated failures in this
// in-memory user chain. It never writes authoritative proxy lifecycle state.
func (c *PoolChain) markFailed(selIdx int, proxyID int) {
	c.failureMu.Lock()
	if c.failures == nil {
		c.failures = make(map[int]int)
	}
	c.failures[proxyID]++
	failureCount := c.failures[proxyID]
	if failureCount >= chainFailureThreshold {
...
func (c *PoolChain) ConnectWithRetry(
	...
	for attempt := 0; attempt < maxAttempts; attempt++ {
		...
		conn, err := connectViaProxyStandalone(ctx, selectedProxy, host, rotationSettings)
		if err != nil {
...
func connectViaProxyStandalone(ctx context.Context, p *models.Proxy, host string, settings *models.RotationSettings) (net.Conn, error) {
