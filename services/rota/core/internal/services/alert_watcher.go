...
// Run watches alert rules until the owning background Group is cancelled.
	...
	for _, rule := range rules {
		...
		if err := w.fire(ctx, rule, *pool); err != nil {
			w.log.Error("failed to fire pool alert webhook",
				"rule_id", rule.ID,
				"url", redactURLForLog(rule.WebhookURL),
				"error", err,
			)
		} else {
			// Record fire time
...
func (w *AlertWatcher) fire(ctx context.Context, rule models.PoolAlertRule, pool models.ProxyPool) error {
	...
	if err != nil {
		return fmt.Errorf("build webhook request for %s: %s", redactURLForLog(rule.WebhookURL), redactURLInError(rule.WebhookURL, err))
	}
	...
	if err != nil {
		return fmt.Errorf("webhook request to %s: %s", redactURLForLog(rule.WebhookURL), redactURLInError(rule.WebhookURL, err))
	}
