    ...
    def test_removes_only_selected_container_preserves_files_and_retries(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            value = dict(nodeId=str(uuid.uuid4()), deploymentId=str(uuid.uuid4()),
                         slot='incremental-2', operationId=str(uuid.uuid4()))
            ...
            compose = {'name': 'qy-node-' + value['nodeId'].replace('-', '')[:16],
                       'services': {'incremental-1': {'image': 'same'}, 'incremental-2': {'image': 'same'}}}
            ...
            labels = {'qy.node.id': value['nodeId'], 'qy.node.slot': value['slot'],
                      'qy.deployment.id': value['deploymentId'], 'qy.remote.mode': 'incremental_collect'}
            present = [True]
            calls = []
            ...
            self.assertEqual(json.loads((deployment / 'compose.json').read_text())['services'], {'incremental-1': {'image': 'same'}})
