...
for(const role of ['incremental','fullcrawl'])test('manager removal is durable, versioned, retries without losing slots, and blocks concurrent deployment',{skip:!url,timeout:20000},async t=>{
  ...
  const node={id:randomUUID(),kind:'execution',name:'fixture',host:'192.0.2.10',port:22,username:'ubuntu',
    workers:[{role,count:3}],provisioning:{state:'ready'},runtime:{state:'ready'},deployment:{state:'connected',mode:role==='fullcrawl'?'full_crawl_collect':'incremental_collect',
  ...
  assert.equal((await post({version:0,slot:`${role==='fullcrawl'?'full-crawl':'incremental'}-2`})).status,409);
  assert.equal((await post({version:1,slot:`${role==='fullcrawl'?'full-crawl':'incremental'}-2`})).status,202);await removal.waitForIdle();
  ...
  assert.equal((await post({version:registry.version,slot:`${role==='fullcrawl'?'full-crawl':'incremental'}-2`})).status,202);await removal.waitForIdle();
  ...
  const args={node:{...registry.nodes[0],workers:[{role,count:4}]},deploymentId:node.deployment.deploymentId,image:node.deployment.image,gatewayUrl:'https://center.example',natsUrl:'tls://messages.example:4222'};
  ...
  await resumed.start({id:node.id,version:registry.version,slot:`${role==='fullcrawl'?'full-crawl':'incremental'}-2`});await resumed.waitForIdle();
  ...
  assert.equal(updated.deployment.appliedCount,2);assert.deepEqual(updated.deployment.slots,[`${role==='fullcrawl'?'full-crawl':'incremental'}-1`,`${role==='fullcrawl'?'full-crawl':'incremental'}-3`]);
  const next=buildNodeCollectDeployment({...args,node:{...updated,workers:[{role,count:3}]}});
  assert.deepEqual(next.slots,[`${role==='fullcrawl'?'full-crawl':'incremental'}-1`,`${role==='fullcrawl'?'full-crawl':'incremental'}-3`,`${role==='fullcrawl'?'full-crawl':'incremental'}-4`]);assert.ok(!next.files[`${role==='fullcrawl'?'full-crawl':'incremental'}-2.json`]);
  assert.equal(next.files[`${role==='fullcrawl'?'full-crawl':'incremental'}-1.json`],buildNodeCollectDeployment(args).files[`${role==='fullcrawl'?'full-crawl':'incremental'}-1.json`]);
