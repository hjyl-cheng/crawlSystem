...
function parseArgs(argv) {
  const options = { apply: false, planDay: null, expectedCount: null, channelIds: [] };
  for (const argument of argv) {
    if (argument === "--apply") options.apply = true;
      ...
      options.channelIds.push(argument.slice(13));
    } else {
...
function databaseConfig(environment = process.env) {
  return {
    ...
    application_name: INCREMENTAL_PARTIAL_RECOVERY_OPERATION_ID,
    max: 2,
  };
}
...
async function main() {
  ...
  try {
    const summary = await recoverIncrementalPartialRuns({
...
main().catch((error) => {
  console.error(JSON.stringify({
    event: "incremental_partial_recovery_failed",
