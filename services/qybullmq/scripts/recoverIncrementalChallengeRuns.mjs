...
import {
  ...
  recoverIncrementalChallengeRuns,
...
function parseArgs(argv) {
  const options = { apply: false, planDay: null, expectedCount: null };
...
function databaseConfig(environment = process.env) {
  return {
    ...
    application_name: INCREMENTAL_CHALLENGE_RECOVERY_OPERATION_ID,
    max: 2,
  };
}
...
async function main() {
  ...
  try {
    const summary = await recoverIncrementalChallengeRuns({
...
main().catch((error) => {
  console.error(JSON.stringify({
    event: "incremental_challenge_recovery_failed",
