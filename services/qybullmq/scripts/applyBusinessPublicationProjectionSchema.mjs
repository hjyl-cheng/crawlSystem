...
export function businessPublicationProjectionSchemaApplyGuard(
  environment = process.env,
  argv = process.argv,
) {
  if (!argv.includes("--apply")) throw new Error("refusing to apply: pass --apply explicitly");
  const confirmedDatabase = String(
    environment.CONFIRM_BUSINESS_PUBLICATION_PROJECTION_SCHEMA_APPLY ?? "",
  ).trim();
  if (!confirmedDatabase) {
    throw new Error(
      "CONFIRM_BUSINESS_PUBLICATION_PROJECTION_SCHEMA_APPLY must equal the target database name",
    );
  }
  const expectedChannelCountValue = String(environment.EXPECTED_BUSINESS_CHANNEL_COUNT ?? "").trim();
...
async function main() {
  const guard = businessPublicationProjectionSchemaApplyGuard();
  ...
  try {
    ...
    const preflight = await client.query(
      `SELECT current_database() AS database_name,
              ...
              to_regclass('publication.projection_outbox') IS NOT NULL AS projection_outbox_ready,
    ...
    assert.equal(actual.projection_outbox_ready, true, "Projection Outbox is missing");
    assert.equal(actual.current_ready, true, "Business Current is missing");
    ...
    console.log(JSON.stringify({
      ...
      migration: "business-publication-projection-schema",
    }));
  } catch (error) {
