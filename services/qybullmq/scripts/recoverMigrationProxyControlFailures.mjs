...
function positiveInteger(value, field, { min = 1, max = 100_000 } = {}) {
...
function parseArgs(argv) {
  ...
  for (const argument of argv) {
    if (argument === "--apply") options.apply = true;
      ...
      options.pollMs = positiveInteger(argument.slice(10), "poll-ms", { min: 1000, max: 300_000 });
    } else {
...
function databaseConfig(environment = process.env) {
  return {
    ...
    application_name: MIGRATION_PROXY_CONTROL_RECOVERY_OPERATION_ID,
    max: 2,
  };
}
...
async function main() {
  ...
  try {
    do {
      ...
      const summary = await recoverMigrationProxyControlFailures({
        query,
        withTransaction,
        queue,
      ...
      console.log(JSON.stringify({
        event: "migration_proxy_control_recovery",
        pressure,
...
main().catch((error) => {
  console.error(JSON.stringify({
    event: "migration_proxy_control_recovery_failed",
