...
assert(
  ...
  "Pass --apply to apply the additive batch control schema",
);
const expected = process.env.EXPECTED_CRAWLER_DATABASE;
assert(
  expected &&
    process.env.CONFIRM_MIGRATION_BATCH_CONTROL_SCHEMA_APPLY === expected,
  "Confirm the expected crawler database",
);
const client = new pg.Client({
  connectionString: databaseUrl(process.env),
  application_name: "migration-batch-control-schema",
});
try {
  ...
  await client.query("SELECT pg_advisory_xact_lock(781137245)");
  const before = await client.query(
    "SELECT count(*)::int AS n FROM crawler.migration_channel_inventory",
  );
  await client.query(
    await readFile(
      new URL("../src/migrationBatchControlSchema.sql", import.meta.url),
      "utf8",
    ),
  );
