...
export function businessPublicationReconciliationSchemaApplyGuard(
  env = process.env,
  argv = process.argv,
) {
  if (!argv.includes("--apply")) throw new Error("refusing to apply: pass --apply explicitly");
  const confirmedDatabase = String(
    env.CONFIRM_BUSINESS_PUBLICATION_RECONCILIATION_SCHEMA_APPLY ?? "",
  ).trim();
  if (!confirmedDatabase) {
    throw new Error(
      "CONFIRM_BUSINESS_PUBLICATION_RECONCILIATION_SCHEMA_APPLY must equal the target database name",
    );
  }
...
async function main() {
  const guard = businessPublicationReconciliationSchemaApplyGuard();
  ...
  try {
    ...
    await client.query("SELECT pg_advisory_xact_lock(781137232)");
    const preflight = await client.query(
      `SELECT current_database() AS database_name,
              ...
              to_regclass('publication.consumer_cursor') IS NOT NULL AS activation_schema_ready,
              to_regclass('result.entity_current') IS NOT NULL AS current_schema_ready,
    ...
    assert.equal(
      actual.activation_schema_ready,
      true,
      "Business Publication Activation schema is missing",
    );
    assert.equal(actual.current_schema_ready, true, "Business Publication Current schema is missing");
    ...
    console.log(JSON.stringify({
      ...
      migration: "business-publication-reconciliation-schema",
    }));
  } catch (error) {
