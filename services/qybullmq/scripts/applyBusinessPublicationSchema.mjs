...
export function businessPublicationSchemaApplyGuard(env = process.env, argv = process.argv) {
  if (!argv.includes("--apply")) throw new Error("refusing to apply: pass --apply explicitly");
  const confirmedDatabase = String(env.CONFIRM_BUSINESS_PUBLICATION_SCHEMA_APPLY ?? "").trim();
  if (!confirmedDatabase) {
    throw new Error("CONFIRM_BUSINESS_PUBLICATION_SCHEMA_APPLY must equal the target database name");
  }
...
async function main() {
  const guard = businessPublicationSchemaApplyGuard();
  ...
  try {
    ...
    await client.query("SELECT pg_advisory_xact_lock(781137230)");
    ...
    console.log(JSON.stringify({
      ...
      migration: "business-publication-schema",
    }));
  } catch (error) {
